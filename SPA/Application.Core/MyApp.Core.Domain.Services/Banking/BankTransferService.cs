    ...
    public class BankTransferService : IBankTransferService
    {
        public void PerformTransfer(decimal amount, BankAccount originAccount, BankAccount destinationAccount)
        {
            ...
            if (originAccount.CanBeWithdrawed(amount))
            {
                ...
                //Charge money
                originAccount.WithdrawMoney(amount, string.Format("messages_TransactionFromMessage", destinationAccount.Id));

                //Credit money
                destinationAccount.DepositMoney(amount, string.Format("messages_TransactionToMessage", originAccount.Id));
            }
            else
